...
class UserProductMvcTest {
    ...
    public void setup() {
        mvc = MockMvcBuilders.webAppContextSetup(context)
                // 클래스 상단에서 제외시킨 WebSecurityConfig 대신 springSecurity(new MockSpringSecurityFilter())를 끼워넣음.
    ...
    void test1() throws Exception {
        // when - then
        mvc.perform(get("/api/user/login-page"))          // GET 요청 /api/user/login-page
                .andExpect(status().isOk())
                .andExpect(view().name("login"))    // 반환될 것으로 예측되는 html 파일명: login
                .andDo(print());
    }
